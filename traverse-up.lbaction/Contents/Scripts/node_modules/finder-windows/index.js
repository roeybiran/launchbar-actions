...
const execf = util.promisify(cp.execFile)
...
module.exports = async (firstOrAll) => {
    ...
    try {
        let stdout = await execf('/usr/bin/osascript', [script, firstOrAll]);
 ...
 * or an Array of the POSIX paths of all Finder window (the front most one will the first member in that array).
 */
module.exports.sync = (firstOrAll) => {
    const script = __dirname + '/bin/finder-windows.scpt';
    try {
        let stdout = cp.execFileSync('/usr/bin/osascript', [script, firstOrAll], { encoding: 'utf-8' } );
        stdout = stdout.trim()
        if (stdout === 'false') {
            return undefined
        } else {
