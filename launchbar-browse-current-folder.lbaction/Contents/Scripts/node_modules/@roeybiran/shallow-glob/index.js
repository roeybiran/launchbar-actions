...
module.exports = async (dir, options) => {
  ...
  if (options) {
    ...
    if (options.ignoreMiscFiles) {
      ignoreMiscFiles = options.ignoreMiscFiles;
    }
  }
  ...
  if (sortBy === "kind") {
    globResults = globResults
      .map(dirent => {
        ...
        const { name } = dirent;
        // prettier-ignore
        // folders with a single . in their name are treated as packages/files
        if ((dirent.isDirectory() && countMatchesIgnoringUndefined(name) === 1 && !name.startsWith(".")) || dirent.isFile()) {
